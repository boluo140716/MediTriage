...
from .extractors import extract_suggestions


class ConsultationAgent(SkillRegistryMixin, BaseAgent):
    """健康咨询 Agent，通过 Skills 调用底层工具。"""
    ...
    def get_system_prompt(self) -> str:
        """获取系统提示词"""
        return f"""你是一位专业的医疗健康咨询顾问。你的职责是提供准确、专业的健康建议和疾病科普。

{identity_and_grounding("MediTriage 医疗健康助手")}
...
7. deep_research: 深度研究（PubMed 文献+本地知识库+证据综合）
...
- 调用 Skill 时，先在正文用一句话说明本次调用的目的（要查什么、为什么），再发起调用
...
"""
    ...
    def format_user_input(self, input_data: Dict[str, Any]) -> str:
        ...
        # 添加上下文信息（如果有）。
        # recent_history 不渲染进 user 消息：同一段历史已由 AgentLoop
        # 以消息级注入，双通道会让模型看到两份并随轮次递归膨胀。
        context = {
            k: v for k, v in (input_data.get('context') or {}).items()
            if k != 'recent_history'
        }
        if context:
            hist_cases = context.pop('historical_cases', None)
            lines = [f"{k}: {v}" for k, v in context.items()]
            if hist_cases:
                lines.append(
                    "相似历史案例（仅供参考；与本次问题无关时请忽略，"
                    "勿与当前患者情况混淆）："
                )
                for h in hist_cases:
                    lines.append(f"- {str(h.get('summary', ''))[:200]}")
            if lines:
                parts.append("背景信息：\n" + "\n".join(lines) + "\n")
    ...
    async def post_process_result(
        ...
        """后处理：从最终响应中提取结构化信息。"""
        # 提取核心建议
        suggestions = extract_suggestions(final_response)

        # 提取免责声明
        disclaimer_pattern = r'【免责声明】\s*\n(.+)'
        disclaimer_match = re.search(disclaimer_pattern, final_response)
        disclaimer = (
            disclaimer_match.group(1) if disclaimer_match
            else "以上信息仅供参考，不能替代专业医生的诊断和治疗。如有疑虑，请及时就医。"
        )
