...
整合本地 Milvus 医学知识库与 PubMed 文献进行深度研究。
"""
...
async def deep_research(query: str, max_iterations: int = 2) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Starting deep research: query={query}, "
        f"max_iterations={max_iterations}"
    )

    # 调用深度研究工作流
    from meditriage.research.deep_research_workflow import DeepResearchWorkflow

    workflow = DeepResearchWorkflow()

    try:
        # 执行研究
        # 注意：DeepResearchWorkflow.run() 返回 ResearchReport 对象，不是 dict
        report = await workflow.run(
            question=query,  # 参数名是 question，不是 query
            max_kb_results=max_iterations * 3  # 用 max_iterations 控制检索结果数
        )
        ...
        return {
            ...
            "confidence": (
                "high" if report.confidence > 0.7
                else "medium" if report.confidence > 0.4
                else "low"
            ),
            "sources": len(report.sources),
            "evidence_level": report.evidence_level,
            "status": "completed",
            "data_sources": "Milvus RAG + PubMed + Evidence Synthesis"
        }
...
def format_research_report(query: str, report) -> str:
    ...
    output.append("\n数据来源：本地知识库（Milvus RAG）+ PubMed 文献 + 证据综合")
