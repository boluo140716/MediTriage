...
async def analyze_symptoms(symptoms: str) -> Dict[str, Any]:
    ...
    symptom_categories = {
        ...
        "musculoskeletal": {
            # 不收泛词"疼痛"——任何含"疼痛"的主诉（如腹部疼痛）都会被
            # 误归到骨骼肌肉系统
            "keywords": ["关节", "肌肉", "骨骼", "腰痛", "背痛", "肿胀", "僵硬"],
            "name": "骨骼肌肉系统"
        }
    }
    ...
    for cat in detected_categories:
        if cat["id"] == "respiratory":
            ...
            # "脑供血不足"非规范诊断名，不列入
            possible_diseases.extend(["偏头痛", "紧张性头痛", "前庭功能紊乱"])
        elif cat["id"] == "musculoskeletal":
            possible_diseases.extend(["骨关节炎", "肌肉劳损", "腰椎间盘突出"])

    # 去重（保序——set 顺序随机会导致同一输入每次输出不同疾病列表）
    possible_diseases = list(dict.fromkeys(possible_diseases))[:5]  # 最多5个
    ...
    return {
        "answer": format_analysis(
            symptoms, patterns, possible_diseases, kb_insights),
        "patterns": patterns,
...
def format_analysis(symptoms: str, patterns: list, diseases: list,
                    kb_insights: list = None) -> str:
    """格式化症状分析结果。"""
    output = [
        f"【症状模式分析】",
        f"\n症状描述：{symptoms}",
