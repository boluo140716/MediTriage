"""LeadAgent：负责任务分解与结果汇总。

注意：LeadAgent 不是编排器。
- 只负责分解任务和汇总结果
- 不控制 Worker 的执行顺序
- 不直接调用 Worker
- 子任务按类型指派给 Worker（assigned_agent），由 SwarmCoordinator 并行调度执行
"""
import uuid
...
class LeadAgent:
    ...
    职责：
    1. 评估问题复杂度
    2. 分解复杂任务为子任务并指派给合适的 Worker（中心化分配）
    3. 等待 Worker 并行完成
    4. 汇总所有结果

    不做：
    - 不干预 Worker 内部的 Skill 选择与推理过程
    """
...
4. 子任务应该相对独立，可以并行执行
5. **ResearchAgent 谨慎使用**：仅在用户明确要求最新研究、临床指南、文献证据、前沿进展时，才分配 ResearchAgent；普通症状咨询、饮食/用药咨询一律不需要 ResearchAgent（用 ConsultationAgent 即可）
...
**适用场景**（仅在用户明确表达深度研究意图时）：
- 需要权威指南（"高血压最新诊疗指南有哪些"）
- 明确要求最新研究进展（"这个病有什么新疗法/临床试验"）
- 需要循证医学证据支持（"有哪些临床证据/文献支持"）
...
"""
    ...
    async def assess_and_decompose(
        ...
        - subtasks: List[SubTask] - 子任务列表
          每个子任务包含：type（工具名）、description（描述）、
        ...
        try:
            # 路由确定化：同输入同分解
            content = await self.llm_client.chat(messages, temperature=0)
            ...
            if json_match:
                try:
                    ...
                    result = {}
                # schema 校验 + 清洗：subtasks 必须是非空 list，
                # 每项含合法 assigned_agent + description
                valid_agents = {
                    "consultation_agent",
                    "diagnostic_agent",
                    "research_agent",
                ...
                if isinstance(raw, list) and raw:
                    ...
                    for st in raw:
                        ...
                        clean.append({
                            "type": st.get("type"),
                            "description": st.get("description") or "回答用户问题",
            ...
            return {
                ...
                "reason": "无法解析/校验 LLM 响应，默认使用 ConsultationAgent"
            }
    ...
    def create_subtasks(
        ...
        for data in subtasks_data:
            # LeadAgent 输出不含 type 字段，按 assigned_agent 推断通用 type
...
如果您的症状严重或紧急，请立即就医或拨打急救电话，不要依赖在线咨询。"""
        ...
        # 构建汇总提示——只取各 Agent 的 answer 正文（整 dict repr 会把
        # iterations/warning 等内部元数据混进汇总 prompt，既费上下文又可能
        # 把内部字段泄漏进最终答案）
        contributions_text = []
        completed_agents = []
        ...
        if timeout_occurred:
            ...
            incomplete_tasks = [
                ...
                if subtask.status.value in ["pending", "in_progress"]
            ]
            if incomplete_tasks:
                timeout_note = f"""
...
**要求**：
1. 综合所有 Agent 的观点
2. 消除各 Agent 结论间的重复与矛盾（不自评"多角度分析更全面可靠"之类的权威性）
3. 保持医疗建议的严谨性
4. 包含【风险评估】【诊断分析】【医学证据】等模块（仅在相关 Agent 真正提供了对应内容时保留）。【医学证据】只在有检索来源时出现，无来源则**整块省略**——不要用"多项文献指出/有研究显示/与专家共识高度一致/具有较强临床指导意义"等泛化权威背书填充，也不要写"证据等级：检索未提供"占位符
5. 给出【核心建议】
6. 添加【免责声明】
