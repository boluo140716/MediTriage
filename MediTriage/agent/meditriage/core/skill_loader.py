"""
Skill Loader 辅助函数
用于动态加载 skills 目录下的 Skill 函数（自动发现）
"""
...
def load_skill_function(
    ...
    spec = importlib.util.spec_from_file_location(
        f"skill_{skill_name.replace('-', '_')}", module_path)
...
def discover_skills(project_root: Path = None) -> List[Dict]:
    ...
    for skill_dir in skills_dir.iterdir():
        ...
        if not script_files:
            logger.warning(
                f"Skipping {skill_name}: no Python script found in script/")
            continue
        ...
        try:
            func = load_skill_function(
                skill_name, script_name, function_name, project_root)
