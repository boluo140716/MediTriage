    ...
    while True:
        try:
            ...
            if user_input.lower() == 'help':
                ...
                print("  clear     - 清屏")
                print("  help      - 显示帮助")
                print("  /image <图片路径> <问题>  - 医学图像问答（VLM）")
                print("  其他输入  - 作为健康问题进行咨询\n")
                continue

            # 处理图像问答命令：/image <path> <question>
            if user_input.startswith('/image'):
                parts = user_input.split(maxsplit=2)
                if len(parts) < 3:
                    print("\n❌ 用法：/image <图片路径> <问题>")
                    print(
                        "   例：/image data/med_image_samples/vqa_rad/"
                        "vqa_rad_000.png 这张影像有什么异常？\n"
                    )
                    continue
                img_path, img_question = parts[1], parts[2]
                conversation_count += 1
                print(f"\n🖼️  医学图像分析中... (第 {conversation_count} 次咨询)\n")
                from meditriage.core.vision_handler import process_image_query
                start_time = time.time()
                img_result = await process_image_query(
                    img_path, img_question, session_id=session_id
                )
                print(f"⏱️  执行时间：{time.time() - start_time:.2f} 秒")
                print("\n📋 影像分析：")
                print("-" * 60)
                print(img_result['answer'])
                print("-" * 60)
                print(f"\n{img_result.get('disclaimer', '')}")
                print("\n" + "=" * 60 + "\n")
                continue
